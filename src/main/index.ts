...
export async function activateMain(registrar: MainRegistrar<'@citadel-app/rss'>, workspace: WorkspaceContext | null) {
    ...
    if (!workspace || !workspace.path) {
        console.warn('[RSS/Main] No workspace — using in-memory SQLite');
        db = new Database(':memory:');
    } else if (workspace.configDir) {
        fs.ensureDirSync(workspace.configDir);
        const dbPath = path.join(workspace.configDir, 'feeds.db');
        console.log(`[RSS/Main] Initializing feed database at: ${dbPath}`);
        db = new Database(dbPath);
    } else {
        console.warn('[RSS/Main] No config dir — using in-memory SQLite');
        db = new Database(':memory:');
    }
    ...
    registrar.handle('getFeedItems', async (feedId: string, limit?: number) => {
        ...
        try {
            ...
            console.error(`[RSS/Main] Failed to get feed items for ${feedId}:`, error);
            return [];
        }
    });
    ...
    registrar.handle('saveFeedItems', async (feedId: string, items: any[]) => {
        ...
        try {
            ...
            console.error(`[RSS/Main] Failed to save feed items for ${feedId}:`, error);
            throw error;
        }
    });
    ...
    registrar.handle('getFeedStatus', async () => {
        ...
        try {
            ...
            console.error('[RSS/Main] Failed to get feed statuses:', error);
            return {};
        }
    });
    ...
    registrar.handle('updateFeedStatus', async (itemId: string, status: any) => {
        ...
        try {
            ...
            console.error(`[RSS/Main] Failed to update feed status for ${itemId}:`, error);
            throw error;
        }
    });

    console.log('[RSS/Main] Feed handlers registered');
}
